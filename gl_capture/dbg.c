...
void DBG_PERF_INIT(const int uid, const int row, const int max) {
	...
	str[uid].row = row;
	str[uid].size = sizeof(LARGE_INTEGER) * row * max;
	str[uid].data = malloc(str[uid].size);
	memset(str[uid].data, 0, str[uid].size);
}
...
void DBG_PERF_FLUSH() {
	...
	for (i = 0; i < DBG_PERF_MAX; i++) {
		...
		fwrite(&str[i].size, 1, 4, f);
		fwrite(str[i].data, 1, str[i].size, f);
		fclose(f);
		free(str[i].data);
	}
}
